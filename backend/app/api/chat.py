# Enhanced chat.py - Complete File with All Current Functionality + All 7 Issues Fixed
...
def safe_float_convert(value: Union[str, int, float, None]) -> float:
    """ENHANCED: Safely convert a value to float with multi-currency support for Issue #2 & #7"""
    if value is None:
        return 0.0
    ...
    try:
        ...
        if isinstance(value, str):
            # ENHANCED: Remove multiple currency symbols and spaces for Issue #7
            cleaned = re.sub(r'[^\d.,-]', '', value)
            if not cleaned:
                return 0.0
                
            # ENHANCED: Handle different decimal separators for international support
            # Convert comma decimal separators to dots (European format)
            if ',' in cleaned and '.' in cleaned:
                # Both comma and dot - assume comma is thousands separator
                cleaned = cleaned.replace(',', '')
            elif ',' in cleaned and cleaned.count(',') == 1:
                # Only one comma - could be decimal separator
                if cleaned.index(',') > len(cleaned) - 4:  # Comma in last 3 positions
                    cleaned = cleaned.replace(',', '.')
                else:  # Comma as thousands separator
                    cleaned = cleaned.replace(',', '')
...
def parse_user_preferences(message: str) -> Dict:
    """ENHANCED: Parse user message with better price pattern support for Issue #7"""
    preferences = {
        'max_results': None,
        'price_filter': None,
        'brand_filter': None,
    ...
    # CRITICAL FIX for Issue #7: Enhanced price patterns with multi-currency support
    price_patterns = [
        # USD patterns
        r'under\s+\$(\d+(?:\.\d{2})?)',
        r'less\s+than\s+\$(\d+(?:\.\d{2})?)',
        r'below\s+\$(\d+(?:\.\d{2})?)',
        r'cheaper\s+than\s+\$(\d+(?:\.\d{2})?)',
        r'budget\s+of\s+\$(\d+(?:\.\d{2})?)',
        r'max\s+\$(\d+(?:\.\d{2})?)',
        r'maximum\s+\$(\d+(?:\.\d{2})?)',
        
        # Indian Rupee patterns (₹ symbol and text)
        r'under\s+₹(\d+(?:\.\d{2})?)',
        r'below\s+₹(\d+(?:\.\d{2})?)',
        r'less\s+than\s+₹(\d+(?:\.\d{2})?)',
        r'cheaper\s+than\s+₹(\d+(?:\.\d{2})?)',
        ...
        # Text-based rupee patterns
        r'under\s+(\d+)\s*(?:rs|rupees?|inr)',
        r'below\s+(\d+)\s*(?:rs|rupees?|inr)',
        r'less\s+than\s+(\d+)\s*(?:rs|rupees?|inr)',
        r'cheaper\s+than\s+(\d+)\s*(?:rs|rupees?|inr)',
        r'budget\s+of\s+(\d+)\s*(?:rs|rupees?|inr)',
        r'max\s+(\d+)\s*(?:rs|rupees?|inr)',
        r'maximum\s+(\d+)\s*(?:rs|rupees?|inr)',
        
        # Generic number patterns (fallback)
        r'(?:products?|items?|things?)\s+under\s+(\d+)',
        r'under\s+(\d+)\s*(?:dollars?|bucks?)',
        r'below\s+(\d+)\s*(?:dollars?|bucks?)',
        
        # Cost-based patterns
        r'cost\s+under\s+[\$₹]?(\d+)',
        r'price\s+under\s+[\$₹]?(\d+)',
        r'priced\s+under\s+[\$₹]?(\d+)'
    ]
    ...
    for pattern in price_patterns:
        ...
        if match:
            ...
            preferences['price_filter'] = {'max': price_value}
            logger.info(f"PRICE FILTER DETECTED: max=${price_value} from pattern: {pattern}")
            break
...
def apply_product_filters(products: List[Dict], filters: Dict, exclude_out_of_stock: bool = True) -> List[Dict]:
    """CRITICAL FIX for Issue #2 & #7: Apply filters with enhanced price filtering and debug logging"""
    if not products:
        return products
    
    filtered = products
    logger.info(f"APPLYING FILTERS: Starting with {len(products)} products")
    logger.info(f"Filters to apply: {filters}")
    
    # ALWAYS filter out-of-stock products unless explicitly disabled
    if exclude_out_of_stock:
        before_stock_filter = len(filtered)
        filtered = [
            p for p in filtered
            if p.get('inventory_quantity', 0) > 0
        ]
        logger.info(f"STOCK FILTER: {before_stock_filter} -> {len(filtered)} products")
    
    # CRITICAL FIX for Issue #7: Enhanced price filter with detailed logging
    if filters and 'price_filter' in filters and filters['price_filter']:
        ...
        if max_price is not None:
            ...
            logger.info(f"PRICE FILTER: Filtering products under ${max_price}")
            
            price_filtered = []
            for p in filtered:
                product_price = safe_float_convert(p.get('price', 0))
                logger.debug(f"Product '{p.get('title', 'Unknown')}': price={product_price}, under_limit={product_price <= max_price}")
                
                if product_price <= max_price:
                    price_filtered.append(p)
            
            filtered = price_filtered
            logger.info(f"PRICE FILTER RESULT: {before_price_filter} -> {len(filtered)} products under ${max_price}")
    
    # Brand filter
    if filters and 'brand_filter' in filters and filters['brand_filter']:
        brand = filters['brand_filter'].lower()
        before_brand_filter = len(filtered)
        filtered = [p for p in filtered if brand in (p.get('vendor', '') or '').lower()]
        logger.info(f"BRAND FILTER: {before_brand_filter} -> {len(filtered)} products for brand '{brand}'")
    
    logger.info(f"FINAL FILTER RESULT: {len(products)} -> {len(filtered)} products")
    return filtered
...
def detect_product_specific_question(message: str, context_product: Optional[Dict] = None,
    ...
    """COMPLETELY FIXED for Issue #1: Better detection of product-specific questions"""
    message_lower = message.lower()
    logger.info(f"CONTEXT ANALYSIS: Analyzing question: '{message}' (lowercased: '{message_lower}')")

    # ENHANCED for Issue #1: Much more comprehensive product question patterns
    product_question_patterns = [
        ...
        r'(?:what\s+(?:is\s+)?(?:the\s+)?|how\s+much\s+(?:does\s+)?(?:is\s+)?(?:this|it)\s+)(?:cost|price)',
        r'(?:how\s+much\s+(?:does\s+)?(?:this|it)\s+cost)',
        r'(?:what\s*\'?s\s+the\s+price\s+(?:of\s+)?(?:this|it)?)',
        r'(?:is\s+there\s+(?:any\s+)?discount\s+(?:on\s+)?(?:this|it))',
        r'(?:any\s+)?(?:deals?|offers?|sales?)\s+(?:on\s+)?(?:this|it)',
        
        # Availability questions
        r'(?:is\s+)?(?:this|it)\s+(?:in\s+stock|available)',
        r'(?:do\s+you\s+have\s+)?(?:this|it)\s+(?:in\s+stock|available)',
        r'(?:is\s+)?(?:this|it)\s+(?:still\s+)?(?:available|in\s+stock)',
        ...
        r'(?:give\s+me\s+)?(?:more\s+)?(?:details?|info|information)\s+(?:about\s+)?(?:this|it)',
        
        # Material and specification questions
        r'(?:what\s+(?:is\s+)?(?:this|it)\s+made\s+(?:of|from))',
        r'(?:what\s+)?material\s+(?:is\s+)?(?:this|it)',
        r'(?:tell\s+me\s+about\s+the\s+)?(?:fabric|material)',
        
        # Enhanced with selection context for Issue #1
        r'(?:give\s+me\s+)?(?:which\s+)?colors?\s+(?:are\s+)?available\s+(?:for\s+|in\s+)?(?:the\s+)?(?:product\s+)?(?:i\s+)?(?:selected|chose)',
        r'(?:give\s+me\s+)?(?:which\s+)?sizes?\s+(?:are\s+)?available\s+(?:for\s+|in\s+)?(?:the\s+)?(?:product\s+)?(?:i\s+)?(?:selected|chose)',
        r'(?:tell\s+me\s+about\s+)?(?:the\s+)?(?:product\s+)?(?:i\s+)?(?:selected|chose)',
        r'(?:what\s+about\s+)?(?:the\s+)?(?:product\s+)?(?:i\s+)?(?:selected|chose)',
        r'(?:show\s+me\s+)?(?:the\s+)?images?\s+(?:of\s+)?(?:the\s+)?(?:product\s+)?(?:i\s+)?(?:selected|chose)',
        
        # Short questions that likely refer to context
        r'^(?:colors?\??|what\s+colors?\?)$',
        r'^(?:sizes?\??|what\s+sizes?\?)$',
        r'^(?:price\??|how\s+much\?)$',
        r'^(?:available\??|in\s+stock\?)$',
        r'^(?:images?\??|photos?\?)$',
        r'^(?:discount\??|any\s+deals?\?)$',
        r'^(?:options?\??|what\s+options?\?)$',
    ]
    ...
    # Check for position-based references first (highest priority)
    position_patterns = [
        r'(?:the\s+)?(first|second|third|fourth|fifth|\d+(?:st|nd|rd|th)?)\s+(?:product|one|item)',
        r'product\s+(?:#|number\s+)?(\d+)',
        r'(?:the\s+)?(?:first|second|third|fourth|fifth|\d+(?:st|nd|rd|th)?)\s+(?:one|item)',
    ...
    for pattern in position_patterns:
        ...
        if match:
            ...
            if position and recent_products and len(recent_products) >= position:
                ...
                logger.info(f"CONTEXT: Found position reference: position {position} -> {target_product.get('title', 'Unknown')}")
                break
    
    # CRITICAL FIX for Issue #1: If no position reference, check selected product FIRST
    if not target_product and selected_product:
        target_product = selected_product
        has_product_reference = True
        logger.info(f"CONTEXT: Using selected product: {target_product.get('title', 'Unknown')}")
    ...
    if not target_product and context_product:
        ...
        logger.info(f"CONTEXT: Using context product: {target_product.get('title', 'Unknown')}, has_reference: {has_product_reference}")
    
    # ENHANCED for Issue #1: Check for product question patterns with better logging
    is_product_question = False
    for i, pattern in enumerate(product_question_patterns):
        if re.search(pattern, message_lower):
            is_product_question = True
            logger.info(f"CONTEXT: Matched product question pattern {i+1}: {pattern}")
            ...
            if 'image' in message_lower or 'photo' in message_lower or 'picture' in message_lower or 'look' in message_lower:
                ...
                question_type = "size"
            elif 'material' in message_lower or 'fabric' in message_lower:
                question_type = "material"
            elif 'discount' in message_lower or 'deal' in message_lower or 'sale' in message_lower:
    ...
    # CRITICAL FIX for Issue #1: If we have a target product but no question pattern match,
    ...
    if not is_product_question and target_product:
        ...
        if any(keyword in message_lower for keyword in product_keywords):
            is_product_question = True
            logger.info("CONTEXT: Detected product question based on keywords and target product presence")
...
async def chat_endpoint(
    ...
    try:
        ...
        # ============================================================================
        # CRITICAL FIX for Issue #1 & #6: PRODUCT CONTEXT PERSISTENCE
        # ============================================================================
        selected_product = None
        
        # STEP 1: Handle new product selection from frontend
        if chat_message.selected_product_id:
            logger.info(f"CONTEXT: Frontend sent selected product ID: {chat_message.selected_product_id}")
            ...
            if new_context_product:
                ...
                session_context[session_id]['selected_product'] = new_context_product
                selected_product = new_context_product
                logger.info(f"CONTEXT: Updated context product to: {new_context_product['title']}")
        
        # STEP 2: CRITICAL FIX for Issue #1 & #6 - If no new selection, use existing context for follow-up questions
        else:
            existing_selected_id = session_context[session_id].get('selected_product_id')
            existing_context_product = session_context[session_id].get('context_product')
            existing_selected_product = session_context[session_id].get('selected_product')
            
            if existing_selected_id and existing_context_product:
                # Use existing context for follow-up questions
                selected_product = existing_selected_product or existing_context_product
                logger.info(f"CONTEXT: Using existing selected product context: {selected_product.get('title', 'Unknown')}")
                
                # Ensure consistency across context variables
        ...
        logger.info(f"CONTEXT STATE FOR ANALYSIS:")
        ...
        logger.info(f"INTENT ANALYSIS: Intent={intent}, Preferences={user_preferences}")
        
        # CRITICAL FIX for Issue #1: Handle product-specific questions FIRST, before other intents
        if product_question_analysis['is_product_question'] and product_question_analysis['should_use_context']:
            ...
            if target_product:
                ...
                # NEW: Handle image requests specifically for better UX
                ...
                # ENHANCED for Issue #4: Generate smart suggestions that avoid the just-asked question
                suggested_questions = generate_contextual_suggestions(target_product, product_question_analysis['question_type'])
            ...
            if is_pagination_request:
                ...
                if chat_message.message.startswith('LOAD_MORE_EXACT_MATCHES: '):
                    ...
                    if cached_query:
                        ...
                        logger.info(f"PAGINATION: Natural language pagination: '{chat_message.message}' -> using cached query '{search_query}'")
                    else:
                        # No cached query - treat as new search
                        search_query = chat_message.message
                        pagination_type = 'exact'
                        is_pagination_request = False  # Reset to new search
                        logger.info(f"PAGINATION: No cached query for natural pagination request, treating as new search")
                else:
                    search_query = chat_message.message
                    pagination_type = 'exact'
            ...
            # NEW: Handle position-based product references
                ...
                if is_pagination_request and cached_results:
                    ...
                    logger.info(f"PAGINATION: Using {len(cached_results)} cached results for pagination")
                    ...
                    if not is_pagination_request and product_results:
                        ...
                        # CRITICAL FIX for Issue #7: Apply filters with proper debugging
                        filters_to_apply = {}
                        if user_preferences.get('price_filter'):
                            filters_to_apply['price_filter'] = user_preferences['price_filter']
                            applied_filters['price_filter'] = user_preferences['price_filter']
                            applied_filters['max'] = user_preferences['price_filter']['max']  # Backward compatibility
                        ...
                        logger.info(f"FILTERS: Applying filters: {filters_to_apply}")
                        ...
                        if not is_pagination_request:
                            ...
                            logger.info(f"CACHE: Cached {len(filtered_products)} products for query: {search_query}")
                        ...
                        logger.info(f"PAGINATION: Using page_size: {page_size} for {total_products} total products")
                        
                        exact_matches = filtered_products[start_idx:end_idx]
                        has_more_exact = end_idx < len(filtered_products)
                        
                        logger.info(f"PAGINATION: Showing products {start_idx+1}-{min(end_idx, len(filtered_products))} of {total_exact_matches} (page {current_page})")
                        ...
                        if exact_matches:
                            ...
                            logger.info(f"CONTEXT: Set new context product: {context_product['title']}")
                        ...
                        if is_pagination_request or (hasattr(locals(), 'is_pagination_request') and locals()['is_pagination_request']):
                            ...
                            # ENHANCED response for Issue #7: Better price-based messaging
                            filter_text = ""
                            if applied_filters:
                                filter_parts = []
                                if 'max' in applied_filters:
                                    currency = "₹" if "₹" in chat_message.message or "rupee" in chat_message.message.lower() else "$"
                                    filter_parts.append(f"under {currency}{applied_filters['max']}")
            ...
            # ENHANCED for Issue #3: General conversation with better relevance
        ...
        # CRITICAL FIX for Issue #3: Always return the current context product in response, but exclude for order/general inquiries
        ...
        # CRITICAL FIX for Issue #3: Only store context product if not an order inquiry or general chat
        ...
        # CRITICAL FIX for Issue #3: Don't include product context for order inquiries or general chat
        ...
        if context_product and intent in ["ORDER_INQUIRY", "GENERAL_CHAT"]:
            logger.info(f"CONTEXT: Excluding context product '{context_product.get('title', 'Unknown')}' from {intent} response")
        elif context_product:
            logger.info(f"CONTEXT: Including context product '{context_product.get('title', 'Unknown')}' in {intent} response")
...
def generate_contextual_suggestions(product: Dict, last_question_type: str) -> List[str]:
    """ENHANCED for Issue #4: Generate intelligent follow-up suggestions based on product and last question"""
    base_suggestions = [
    ...
    # CRITICAL FIX for Issue #4: Remove the question type they just asked about to avoid repetition
    ...
    # Return up to 4 relevant suggestions
